...
public class ChatSettingsGui {
    ...
    // -------------------------------------------------------------------------
    private final float[] navHover   = new float[6];
    ...
    // Keybinds page state
    private GuiTextField kbMessageField;
    private GuiTextField kbLabelField;
    private GuiTextField arTriggerField;
    private GuiTextField arResponseField;
    private boolean      recordingKeybind   = false;
    private final java.util.List<Integer> pendingKeybindCodes = new java.util.ArrayList<>();
    ...
    private static final String[] PAGE_NAMES  = {"Appearance", "Filters", "Search", "Keybinds", "Mutes", "Help"};
    ...
    public void draw(int mx, int my) {
        ...
        else if (currentPage == 3) drawKeybindsPage(mc, cx, cy, mx, my);
        else if (currentPage == 4) drawMutesPage(mc, cx, cy, mx, my);
    ...
    private void drawFiltersPage(Minecraft mc, int cx, int cy, int mx, int my) {
        ...
        if (data.showNotifications) {
            int indX = cx + 16;
            int indW = CW - 16;
            // Sound notifications (dimmed if windows is active)
            ...
            { boolean hov = !soundDimmed && mx>=indX && mx<=indX+indW && my>=cy && my<=cy+14;
              ...
              mc.fontRendererObj.drawString("Sound Notification",
                      indX+7, cy+4, soundDimmed ? C_TEXT_DIM : C_TEXT);
              cy += 16;
            }
            // Windows notifications (dimmed if sound is active)
            ...
            { boolean hov = !winDimmed && mx>=indX && mx<=indX+indW && my>=cy && my<=cy+14;
              ...
              mc.fontRendererObj.drawString("Windows Notification",
                      indX+7, cy+4, winDimmed ? C_TEXT_DIM : C_TEXT);
              cy += 16;
            }
        }
    }
    ...
    /** Returns true while waiting for a key combo to be recorded. */
    public boolean isRecordingKeybind() { return recordingKeybind; }

    private String buildComboString() {        StringBuilder sb = new StringBuilder();
    ...
    // Keybinds page
    // -------------------------------------------------------------------------
    private void drawKeybindsPage(Minecraft mc, int cx, int cy, int mx, int my) {
        ScaledResolution sr = new ScaledResolution(mc);
        int panelY = (sr.getScaledHeight() - H) / 2;
        ...
        cy += 14; // extra padding so description doesn't clip into fields

        // Label + Message fields (side by side, full width)
        int halfFW = CW / 2 - 2;
        ...
        // Record key button — shows current recorded combo or "Press keys..." while recording
        int recW = CW / 2 - 2;
        ...
        for (int i = kbScrollY; i < data.keybinds.size() && ry < cy+kbListAreaH; i++) {
            ...
            Gui.drawRect(cx, ry, cx+CW, ry+kbRowH, rHov ? C_CARD_H : (i%2==0?C_CARD:0xFF101418));
        ...
        for (int i = arScrollY; i < data.autoResponses.size() && ry < cy+arListAreaH; i++) {
            ...
            Gui.drawRect(cx, ry, cx+CW, ry+arRowH, rHov ? C_CARD_H : (i%2==0?C_CARD:0xFF101418));
            String trig = ar.trigger; if (mc.fontRendererObj.getStringWidth(trig)>CW/2-12) { while(mc.fontRendererObj.getStringWidth(trig+"…")>CW/2-12 && trig.length()>1) trig=trig.substring(0,trig.length()-1); trig+="…"; }
            String resp = ar.response; if (mc.fontRendererObj.getStringWidth(resp)>CW/2-20) { while(mc.fontRendererObj.getStringWidth(resp+"…")>CW/2-20 && resp.length()>1) resp=resp.substring(0,resp.length()-1); resp+="…"; }
    ...
    public void mouseClicked(int mx, int my, int btn) {
        ...
        if (currentPage == 0) {
            ...
            // Notifications section (cy already past message types)
            cy += 5 + 13; // gap + section header
            // Master toggle
            if (btn==0 && mx>=cx && mx<=cx+CW && my>=cy && my<=cy+14) {
                data.showNotifications = !data.showNotifications;
                if (!data.showNotifications) {
                    data.soundNotifications   = false;
                    data.windowsNotifications = false;
                }
                data.save(); return;
            }
            cy += 16;
            if (data.showNotifications) {
                ...
                // Sound — mutually exclusive with Windows
                ...
                cy += 16;
                // Windows — mutually exclusive with Sound
                if (btn==0 && mx>=indX && mx<=indX+indW && my>=cy && my<=cy+14) {
            ...
            // ── Keybinds page ──
            // Mirror drawKeybindsPage cy advances exactly:
            cy += 13;  // section header
            cy += 14;  // description + padding
            cy += 10;  // field label row
            // fields are drawn at cy, then cy += 18
            kbLabelField.mouseClicked(mx, my, btn);
            kbMessageField.mouseClicked(mx, my, btn);
            cy += 18;  // field height (14) + gap (4)

            int recW = CW / 2 - 2;

            // Record Key Combo button — always clickable (toggles recording on/off)
            if (btn == 0 && mx >= cx && mx <= cx + recW && my >= cy && my <= cy + 14) {
                if (recordingKeybind) {
                    recordingKeybind = false;  // confirm / stop recording
                } else {
            ...
            int arRowH = 13;
            ry = cy;
            int arListAreaH = H - (cy - (sr.getScaledHeight()-H)/2) - 8;
            for (int i = arScrollY; i < data.autoResponses.size() && ry < cy+arListAreaH; i++) {
                int delX2 = cx+CW-12;
        ...
        // Help page (currentPage==5) has no interactive elements
        data.save();
    }
    ...
    public void keyTyped(char c, int code) {
        ...
        // Keybinds page — capture keys for recording, or type into fields
        if (currentPage == 3) {
            if (recordingKeybind) {
                if (code == 1 /*KEY_ESCAPE*/) {
                    recordingKeybind = false;
                    pendingKeybindCodes.clear();
                    return;
                }
                // Accumulate the key if not already in list
                if (code != 0 && !pendingKeybindCodes.contains(code)) {
                    pendingKeybindCodes.add(code);
                }
                return; // swallow all keystrokes while recording
            }
            ...
            if (arResponseField.isFocused()) { arResponseField.textboxKeyTyped(c, code); return; }
        }
        // Mutes page
        if (currentPage == 4 && muteAddField.isFocused()) {
